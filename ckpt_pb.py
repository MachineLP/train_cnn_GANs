...
from lib.utils.GANs_utils import build_generator
import cv2
import numpy as np
import os
import sys
...
if arch_model == "arch_dcgan_unconditional":
    X = tf.placeholder(tf.float32, [None, dim], name = "inputs_placeholder")
    with tf.variable_scope("Generator_dcgan", reuse=None) as scope:
        net, net_vis = build_generator(X, num_classes, 1.0, False, arch_model)
elif arch_model == "arch_dcgan_conditional":
    X = tf.placeholder(tf.float32, [None, dim], name = "inputs_placeholder0")
    Y = tf.placeholder(tf.float32, [None, num_classes], name = "inputs_placeholder1")
    with tf.variable_scope("Generator_dcgan", reuse=None) as scope:
        net, net_vis = build_generator([X,Y], num_classes, 1.0, False, arch_model)
else:
    print ('{} is error!', arch_model)
# net = tf.nn.softmax(net)
# predict = tf.reshape(net, [-1, num_classes], name='predictions')
predict = tf.reshape(net, [-1, height, width, 3], name='predictions')
...
def freeze_graph(model_folder):
    ...
    with tf.Session() as sess:
        ...
        if arch_model == "arch_dcgan_unconditional":
            ...
            pred = graph.get_tensor_by_name('prefix/inputs_placeholder0:0')
