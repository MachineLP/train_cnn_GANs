...
def train_GANs(train_data,train_label,valid_data,valid_label,train_dir,num_classes,batch_size,arch_model,learning_r_decay,learning_rate_base,decay_rate,dropout_prob,epoch,height,width,checkpoint_exclude_scopes,early_stop,EARLY_STOP_PATIENCE,fine_tune,train_all_layers,checkpoint_path,train_n,valid_n,g_parameter,dim = 64):
    ...
    G_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'Generator_dcgan')
    # print (G_vars)
    D_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'Discriminator_dcgan')
    # print (G_vars)
    ...
    if arch_model == "arch_dcgan_unconditional":
        ...
        G_optimizer = train_op(learning_rate, G_loss, G_vars, global_step)
        D_optimizer = train_op(learning_rate*0.01, D_loss, D_vars, global_step)
    else:
        print ('{} is error!', arch_model)
    ...
    for epoch_i in range(epoch):
        for batch_i in range(int(train_n/batch_size)):
            ...
            if arch_model == "arch_dcgan_unconditional":
                ...
                G_los, _ = sess.run([G_loss,G_optimizer], feed_dict={G_X: generator_x,G_Y:labels,G_is_train:True, G_keep_prob_fc:dropout_prob})
            else:
                print ('{} is error!', arch_model)
            print ('D_los:', D_los)
            print ('G_los:', G_los)
            ...
            if batch_i%20==0:
                if arch_model == "arch_dcgan_unconditional":
                    D_loss_ = sess.run(D_loss, feed_dict={G_X: generator_x, G_is_train:True, G_keep_prob_fc:1.0,D_X: images,  D_is_train:True, D_keep_prob_fc:1.0})
                    G_loss_ = sess.run(G_loss, feed_dict={G_X: generator_x,  G_is_train:True, G_keep_prob_fc:1.0})
                elif arch_model == "arch_dcgan_conditional":
                    D_loss_ = sess.run(D_loss, feed_dict={G_X: generator_x,G_Y:labels,G_is_train:True,G_keep_prob_fc:1.0,D_X: images,D_Y:labels,D_is_train:True, D_keep_prob_fc:1.0})
                    G_loss_ = sess.run(G_loss, feed_dict={G_X: generator_x,G_Y:labels,G_is_train:True, G_keep_prob_fc:1.0})
                else:
                    print ('{} is error!', arch_model)
            ...
            if batch_i%100==0:
                ...
                if arch_model == "arch_dcgan_unconditional":
                    ...
                    G_ls = sess.run(G_loss, feed_dict={G_X: generator_x,G_Y:labels,G_is_train:False, G_keep_prob_fc:1.0})
                else:
                    print ('{} is error!', arch_model)
        ...
        for batch_i in range(int(valid_n/batch_size)):
            ...
            if arch_model == "arch_dcgan_unconditional":
                ...
                images, labels = get_next_batch_from_path(valid_data, valid_label, batch_i%(int(valid_n/batch_size)), height, width, batch_size=batch_size, training=False)
                G_epoch_ls, G_samples = sess.run([G_loss, G_net], feed_dict={G_X: generator_x, G_Y:labels, G_keep_prob_fc:1.0, G_is_train:False})
