...
from lib.model.generator.build_generator.build_generator import generator_arch
from lib.model.discriminator.build_discriminator.build_discriminator import discriminator_arch
from lib.model.build_net import networks
import tensorflow as tf
...
#######
def generator_input_placeholder(dim, num_classes):
...
def discriminator_input_placeholder(height, width, num_classes):
...
def build_generator(X, num_classes, keep_prob_fc, is_train, arch_model):
    arch = networks
    ...
    elif arch_model == "arch_dcgan_unconditional":
        net, net_vis = arch.generator(X, is_training=is_train)
    elif arch_model == "arch_dcgan_conditional":
        net, net_vis = arch.conditional_generator(X, is_training=is_train)
    else:
        print ('{} is error!', arch_model)
    return net, net_vis


def build_discriminator(X, num_classes, keep_prob_fc, is_train, arch_model):
    arch = networks
    if arch_model == "arch_inception_v4_multi_label":
        net, net_vis = arch.arch_inception_v4(X, num_classes, keep_prob_fc, is_train)
    elif arch_model == "arch_inception_v4_rnn_multi_label":
        ...
        net, net_vis = arch.discriminator(X, is_training=is_train)
    elif arch_model == "arch_dcgan_conditional":
        net, net_vis = arch.conditional_discriminator(X[0], [X[0],X[1]], is_training=is_train)
    else:
        print ('{} is error!', arch_model)
    return net, net_vis
